...
open class MyApplication : Application(), SharedPreferences.OnSharedPreferenceChangeListener,
    ...
    fun getAppDisplayName(): String {
        return "Ta Tenu Grana"
    }
    

    fun getOriginalCredits(): String {
        return "Ta Tenu Grana é baseado no MyExpenses\n" +
               "desenvolvido por Michael Totschnig\n\n" +
               "Projeto original: https://github.com/mtotschnig/MyExpenses\n" +
               "Licença: GPL v3+"
    }
    ...
    fun isSyncEnabled(): Boolean {
        return try {
            FirebaseSync.getInstance().isEnabled()
        } catch (e: Exception) {
            false
        }
    }
